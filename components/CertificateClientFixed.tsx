                ...
                {descValue && (
                  <div className="mt-3 text-gray-700">
                    <div className="max-w-none" style={isExpanded ? { overflow: 'visible', maxHeight: 'none' } as React.CSSProperties : { overflow: 'hidden', maxHeight: '6.5rem' } as React.CSSProperties}>
                      {!isExpanded ? (
                        <div className="text-gray-700 portable-text-collapse">{toPlainFirstParagraph(descValue) || toPlainWords(descValue, 30)}</div>
                      ) : (
                    ...
                    <div className="mt-2 flex items-center justify-between gap-4">
                      <div className="flex-1 flex justify-end">
                        <button id={`cert-toggle-${cert._id}`} aria-expanded={isExpanded} aria-controls={`cert-desc-${cert._id}`} onClick={(ev) => { ev.stopPropagation(); toggle(cert._id); }} className={`text-sm font-semibold mt-2 text-red-600`}>
