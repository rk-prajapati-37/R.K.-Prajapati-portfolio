// Re-export the cleaned fixed Education client
export { default } from "./EducationClientFixed";
// Re-export the cleaned fixed Education client
export { default } from "./EducationClientFixed";
export { default } from "./EducationClientFixed";
"use client";
...
const item = { hidden: { opacity: 0, y: 16 }, show: { opacity: 1, y: 0, transition: { duration: 0.45 } } };
...
export default function EducationClient({ educations }: { educations: Education[] }) {
  ...
  const toggle = (id: string) => setExpandedId((prev) => (prev === id ? null : id));
  const formatDate = (d?: string) => { if (!d) return ""; try { return new Date(d).toLocaleDateString("en-US", { year: "numeric", month: "short" }); } catch { return d; } };
  ...
  return (
    <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.2 }} className="space-y-6">
      {sorted.map((edu) => {
        ...
        const desc = edu.description || (edu as any).desc || (edu as any).content;
        return (
          <motion.div key={edu._id} variants={item} className="card p-6 rounded-lg shadow-md border-l-4 border-red-500">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                <h3 className="text-lg font-semibold">{edu.degree}</h3>
                <div className="flex items-center gap-3 mt-1">
                  {edu.logo ? <img src={edu.logo} alt={`${edu.institution} logo`} width={36} height={36} className="rounded-full" /> : null}
                  <div>
                    <div className="text-sm text-gray-700 font-medium">{edu.institution}</div>
                    <div className="text-xs text-gray-500">{formatDate(edu.startDate)} - {formatDate(edu.endDate)}</div>
                  </div>
...
export default function EducationClient({ educations }: { educations: Education[] }) {
  ...
  return (
    <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.2 }} className="space-y-6">
      {sorted.map((edu) => {
        const isExpanded = expandedId === edu._id;
        const desc = edu.description || edu['desc'] || edu['content'];
        return (
          <motion.div key={edu._id} variants={item} className="card p-6 rounded-lg shadow-md border-l-4 border-red-500">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
