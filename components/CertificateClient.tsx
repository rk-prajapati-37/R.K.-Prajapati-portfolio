...
export default function CertificateClient({ certificates }: { certificates: Certificate[] }) {
  ...
  return (
    <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.2 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {sortedCerts.map((cert) => {
        const isExpanded = expandedId === cert._id;
        const descValue = (cert as any).description ?? (cert as any).desc ?? (cert as any).content;
        return (
          <motion.div key={cert._id} variants={item}>
            <div className={`certificate-card flex flex-col md:flex-row gap-4 p-4 rounded-lg shadow-sm transition hover:shadow-lg ${isExpanded ? 'ring-2 ring-red-500' : ''}`}>
              <div className="w-full md:w-36 h-40 flex-shrink-0 overflow-hidden rounded bg-gray-100 flex items-center justify-center">
                {cert.certificateImage ? (
              ...
              <div className="flex-1 flex flex-col">
                ...
                {descValue && (
                  <div className="mt-3 text-gray-700">
                    <div className={`portable-text-collapse ${isExpanded ? 'expanded' : ''}`}>
                      {!isExpanded ? (
                        <div style={{ whiteSpace: 'pre-wrap' }}>{toPlainFirstParagraph(descValue) || toPlainWords(descValue, 30)}</div>
                      ) : (
                        <div id={`cert-desc-${cert._id}`} className="portable-text" style={{ overflow: 'visible' }}>
                          <PortableTextClient value={descValue} />
                        </div>
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      <button onClick={(ev) => { ev.stopPropagation(); toggleExpanded(cert._id); }} className="text-sm font-semibold text-red-600">{isExpanded ? 'show less' : 'show more'}</button>
                      {cert.url && (
                        <a href={cert.url} target="_blank" rel="noopener noreferrer" className="inline-block bg-red-600 text-white px-3 py-1 rounded text-sm">View</a>
                      )}
                    </div>
                  </div>
                )}
        ...
        return (
          <motion.div key={cert._id} variants={item}>
            <div onClick={(e) => handleCardClick(e, cert._id)} aria-expanded={isExpanded} aria-controls={`cert-desc-${cert._id}`} className={'certificate-card card rounded-lg shadow-md p-4 hover:shadow-lg transition transform hover:-translate-y-0.5 h-full flex flex-col md:flex-row md:items-start gap-4 min-h-0 cursor-pointer' + (isExpanded ? ' ring-2 ring-red-500' : '')}>
              ...
              {/* CTA already shown above; removed duplicate link to avoid repetition */}
            </div>
          </motion.div>
        );
      })}
