...
export default function PortableTextClient({ 
  ...
  // If truncation requested, return plain text
  if (truncate) {
    if (isString) {
      return (
  ...
  const components = {
    block: {
      ...
      blockquote: ({ children }: any) => (
        <blockquote className="pl-4 border-l-2 border-gray-300">{children}</blockquote>
      ),
    },
    list: {
  ...
  return (
    <div className={`${className || ''} prose max-w-none`} style={{ color: "var(--text)" }}>
      {isString ? (
        <div>
